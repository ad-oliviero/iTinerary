...
class PlacesAPIRequest: GeoRequest {
    ...
    var conditions: [Condition] = [.access]
    /// Max amount of results
    var limit: Int
    /// Kind of like page 1, 2, etc...
    var offset: Int
    init(placeId: String!, categories: [Category]!, conditions: [Condition] = [], limit: Int = 20, offset: Int = 0) {
        ...
        super.apiVersion = "v2/"
        apiType = "places?"
        parameters = "filter=place:\(self.placeId)&categories=access,\(self.categories.map { $0.rawValue }.joined(separator: ","))&conditions=\(self.conditions.map { $0.rawValue }.joined(separator: ","))&limit=\(self.limit)&offset=\(self.offset)"
    }
