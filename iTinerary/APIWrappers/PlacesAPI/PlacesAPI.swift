...
class PlacesAPIRequest: GeoRequest {
  ...
  init(
    ...
    apiType = "places?"
    parameters =
      "filter=place:\(self.placeId)&categories=access,\(self.categories.map { $0.rawValue }.joined(separator: ","))&conditions=\(self.conditions.map { $0.rawValue }.joined(separator: ","))&limit=\(self.limit)&offset=\(self.offset)"
  }
